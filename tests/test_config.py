...
def test_packaged_config_resolves_four_g1_policies():
    ...
    assert {item.spec.name: item.kind for item in resolved} == {
        "g1-fastsac-locomotion": "locomotion",
        "g1-ppo-locomotion": "locomotion",
        "g1-fastsac-wbt-dancing": "wbt",
        "g1-ppo-wbt-dancing": "wbt",
    }
...
def test_packaged_policy_and_mqtt_yaml_expose_every_field():
    ...
    for policy_path in policy_paths:
        ...
        assert set(policy["task"]) == {field.name for field in fields(TaskConfig)}
...
def test_config_rejects_duplicates_and_composed_policies():
    runtime, path = load_runtime_config()
    ...
    base["name"] = "arms"
    base["type"] = "upper_body"
    composed = PolicySpec.model_validate(base)
    with pytest.raises(ValueError, match="full_body"):
        RuntimeConfig(robot=runtime.robot, mqtt=runtime.mqtt, policies=(composed,))
...
def test_model_override_accepts_absolute_path(tmp_path):
    ...
    data = yaml.safe_load((default_path / "fastsac_locomotion.yaml").read_text(encoding="utf-8"))
    data["name"] = "custom"
...
def test_model_path_rejects_remote_sources(model_path):
    ...
    data = yaml.safe_load((path / "fastsac_locomotion.yaml").read_text(encoding="utf-8"))
    data["task"] = task
