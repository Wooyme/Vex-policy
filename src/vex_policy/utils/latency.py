...
class LatencyStage(StrEnum):
    """Measurement stages in report order, with stable names for logging."""
...
class LatencyTracker:
    ...
    def __init__(self, window_size: int = 50):
        ...
        self.measurements: dict[LatencyStage, deque[float]] = defaultdict(lambda: deque(maxlen=window_size))
        self.current_cycle: dict[LatencyStage, float] = {}
    ...
    @contextmanager
    def measure(self, stage: LatencyStage):
    ...
    def end_cycle(self) -> dict[LatencyStage, float]:
        """End current cycle and return measurements."""
        if self.cycle_start_time:
            total_time = (time.perf_counter() - self.cycle_start_time) * 1000
            self.current_cycle[LatencyStage.TOTAL] = total_time
            self.measurements[LatencyStage.TOTAL].append(total_time)

        return self.current_cycle.copy()

    def get_stats(self, stages: list[LatencyStage] | None = None) -> dict[LatencyStage, LatencyStats]:
